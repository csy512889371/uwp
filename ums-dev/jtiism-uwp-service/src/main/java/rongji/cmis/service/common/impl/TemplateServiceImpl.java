...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			File envXmlFile = new ClassPathResource(CommonAttributes.CMIS_XML_PATH).getFile();
			Document document = new SAXReader().read(envXmlFile);
			List<Template> templates = new ArrayList<Template>();
			List<Element> elements = document.selectNodes("/cmis/template");
	...
	public List<Template> getList(Template.Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(envXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/cmis/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File envXmlFile = new ClassPathResource(CommonAttributes.CMIS_XML_PATH).getFile();
			Document document = new SAXReader().read(envXmlFile);
			Element element = (Element) document.selectSingleNode("/cmis/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
