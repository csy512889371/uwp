...
<body>
	<form action="<%=basePath%>sys/version/saveVersionPublicUpdate.do" method="post" id="updateVersionPublicForm">
		<input type="hidden" name="selectedSeqnos" id="selectedSeqnos">
		<input type="hidden" name="scriptType" id="scriptType" value="${versionPublic.scriptType}">
		<input type="hidden" name="publicId" id="publicId" value="${versionPublic.publicId}">
		<div class="table_div">
			<table class="table table-bordered">
				<tr>
					<td style="width: 7%;"><span style="font-size:15px;color:red;">*</span>版本号：</td>
					<td style="width: 26%;">
						<input type="text" name="versionNo" id="versionNo" value="${versionPublic.versionNo}" maxlength="20" class="required">
					</td>
					<td style="width: 7%;"><span style="font-size:15px;color:red;">*</span>发布日期：</td>
					<td style="width: 26%;">
						<input type="text" name="pubDate" id="pubDate" value="<fmt:formatDate value="${versionPublic.pubDate}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>" onfocus="getDate(this,'yyyy-MM-dd HH:mm:ss')" class="required">
					</td>
					...
					<td style="width: 26%;">
						<select id="pubType" name="pubType" class="required" onchange="loadScriptVersion()">
							...
							<option value="0" <c:if test="${versionPublic.pubType == 0 }">selected="selected"</c:if>>发布一</option>
							<option value="1" <c:if test="${versionPublic.pubType == 1 }">selected="selected"</c:if>>发布二</option>
							<option value="100" <c:if test="${versionPublic.pubType == 100 }">selected="selected"</c:if>>发布三</option>
						</select>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td>发布说明：</td>
					<td>
						<textarea rows="3" cols="20" name="pubDesc" id="pubDesc" style="margin: 0px; width: 300px; height: 50px;" maxlength="500">${versionPublic.pubDesc}</textarea>
					</td>
					<td>备注：</td>
					<td>
						<textarea rows="3" cols="20" name="memo" id="memo" style="margin: 0px; width: 300px; height: 50px;" maxlength="500">${versionPublic.memo}</textarea>
					</td>
	...
	<script>
		$(document).ready(function() {
			...
			loadScriptVersion();
			$("#updateVersionPublicForm").validate({
		...
		function updateVesionPublic(){
			...
			$('#updateVersionPublicForm').ajaxSubmit(function(data){
				closeWindow();
		...
		function loadScriptVersion(){
			//$('#alert_dialog_content').css('text-align','left');
			...
			$.ajax({
				type : 'post',
				url  : '<%=basePath%>sys/version/getUpdateScriptVersionList.do',
				data : {pubType : pubType,publicId: publicId},
				success : function(data){
					...
					for(var i in data){
						...
						tb = tb + '<tr style="height:30px;">';
						if (1 == data[i].isSelected) {
							tb = tb + '<td><label><input type="checkbox" name="selectedSeqno" checked="checked" id="selectedSeqno" value="'+data[i].seqno+'"></label></td>';
						} else {
		...
		function getChnScript(seqNo) {
			...
			$.ajax({
				...
				success : function(data){
					//$('#alert_dialog').attr("style","text-align:left;padding:5px;");
				    ...
				    $("#alert_dialog").dialog({
				        ...
				        buttons: {
				            "确定": function () {
				                ...
				                //$("#alert_dialog").attr("style","text-align: center; padding: 50px 5px 5px;");
				            }
				        }
				    });
				}
