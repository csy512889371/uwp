...
<div style="text-align: center;">
    <table class="table table-bordered" style="margin-bottom: 10px;padding-bottom: 0px;">
        ...
        <c:if test="${type=='bySelEnterprise'}">
            <tr>
                <td>
                    &nbsp;导出范围 &nbsp;&nbsp;
                    <label class="radio inline"><input type="radio" name="enterpriseSelected" value="selected"
                                                       checked="checked">选择的企业</label>&nbsp;&nbsp;
    ...
    <div class="right_btn" style="margin-right: 20px;padding-top: 0px;">
        ...
        <input class="btn-style1" style="width:60px;" type="button" onclick="closeWindow(1)" value="取消">
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.type == "success") {
                    closeWindow(1);
                    if (result.content != null && $.trim(result.content) != '') {
                        var fileName = encodeURI(encodeURI(result.content));
                        var relFolderName = encodeURI(encodeURI('${setting:get().dynaReportTempDoc}'));
                        if (!!window.ActiveXObject || "ActiveXObject" in window) {
                            window.location.href=$('#loadFileOfCadreInfo').attr("action") + "?fileName=" + fileName + "&relFolderName=" + relFolderName;
                        }
                        else {
                            $('#loadfileNameOfCadreInfo').val(fileName);
                            $('#loadRelFolderNameOfInfo').val(relFolderName);
                            $('#loadFileOfCadreInfo').submit();
                        }
                    } else {
                        openAlert("文件未生成");
