    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            var mmGrid = [
                '<div class="mmGrid">',
                    ...
                    '<div class="mmg-message nodata-mmg-message">'+ this.opts.noDataText +'</div>',
            ...
            if(opts.checkCol){
            	...
            	if(opts.multiSelect){
            		if(opts.checkAll) {
            			chkHtml = '<input type="checkbox" class="checkAll" >';
            		}
            	} 
            	
                //<input type="radio" disabled="disabled" class="checkAll">
                opts.cols.unshift({title:chkHtml,width: 20, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
                	return opts.multiSelect ? '<input type="checkbox" class="mmg-check">' : '<input type="radio" class="mmg-check">'
                }});
            }
        ...
        , _initEvents: function(){
            ...
            $body.on('dblclick','td',function(e){
            	clearTimeout(_time);
                var $this = $(this);
                var event = jQuery.Event("dbCellClicked");
                event.target = e.target;
                that.$body.triggerHandler(event, [$.data($this.parent()[0], 'item'), $this.parent().index(), $this.index()]);
            ...
            $body.on('click','tr > td .mmg-check',function(e){
            	...
            	var $this = $(this);
            	 var event = jQuery.Event("checkboxSelected");
                 event.target = e.target;
                 that.$body.triggerHandler(event, [$.data($this.parents('tr')[0], 'item'), $this.parents('tr').index()]);
                 if(event.isPropagationStopped()){
                     return;
                 }
            ...
            $body.on('click','input[type="radio"] ',function(e){
               ...
               _time = setTimeout(function(){
                    event.target = e.target;
                    console.log($.data($this.parent()[0], 'item'));
                    that.$body.triggerHandler(event, [$.data($this.parents('tr')[0], 'item'), $this.parents('tr').index(), $this.parents('td').index()]);

                    if(event.isPropagationStopped()){
                        return;
                    }
                    if(!$this.parent().hasClass('selected')){
                        that.select($this.parents('tr').index());
                    }else{
                        that.deselect($this.parents('tr').index());
                    }
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                var strs = '';
                if(rowIndex%2==1){//斑马线
                	//trHtml.push('<tr class="rows_color">');
                	strs = 'rows_color';
                }else{
                	//trHtml.push('<tr>');
                }
        ...
        , updateRowFast: function(item, index){
        	var $tbody = this.$body.find('tbody');
            var $tr = $tbody.find('tr').eq(index);
            $tr.html(this._rowHtml(item, index).slice(4,-5));
        }
      //禁用行，参数可以为索引数组
        , disableRow: function(index){
            ...
            //var checked = $tr.find('td:first :checkbox').is(':checked');
            if(opts.checkCol){
                $tr.find('td:first :checkbox').prop('checked',true);
                $tr.find('td:first :checkbox').attr("disabled",true);
            }
