 ...
 */
(function(g) {
	var A = g.Chart, t = g.addEvent, B = g.removeEvent, C = HighchartsAdapter.fireEvent, j = g.createElement, p = g.discardElement, v = g.css, l = g.merge, m = g.each, q = g.extend, E = g.splat, F = Math.max, k = document, D = window, G = g.isTouchDevice, H = g.Renderer.prototype.symbols, r = g
			.getOptions(), y;
	q(r.lang, {
		printChart : "打印报表",
		downloadPNG : "导出PNG格式报表",
		downloadJPEG : "导出JPEG格式报表",
		downloadPDF : "导出PDF格式报表",
		downloadSVG : "导出SVG格式报表",
		contextButtonTitle : "导出报表"
	});
	r.navigation = {
		menuStyle : {
	...
	q(
			A.prototype,
			{
				...
				print : function() {
					...
					if (!b.isPrinting)
						b.isPrinting = !0, C(b, "beforePrint"), m(g, function(
								a, b) {
							if (a.nodeType === 1)
								e[b] = a.style.display,
										a.style.display = "none"
						}), f.appendChild(a), D.focus(), D.print(), setTimeout(
								function() {
									c.appendChild(a);
									m(g, function(a, b) {
										if (a.nodeType === 1)
				...
				contextMenu : function(b, a, e, c, f, g, h) {
					var d = this, s = d.options.navigation, l = s.menuItemStyle, n = d.chartWidth, o = d.chartHeight, k = "cache-"
							+ b, i = d[k], u = F(f, g), w, x, p, r = function(a) {
					...
					if (!i)
								...
								m(
										...
										function(a) {
											if (a) {
												var b = a.separator ? j("hr", null, null, w) : j(
																"div",
																{
																	onmouseover : function() {
																		v(
																				this,
																				s.menuItemHoverStyle)
																	},
																	onmouseout : function() {
																		v(this,
																				l)
					...
					e + d.exportMenuWidth > n ? a.right = n - e - f - u + "px"
							: a.left = e - u + "px";
				...
				addButton : function(b) {
					...
					if (c.enabled !== !1) {
						...
						d.menuClassName = b.menuClassName || "highcharts-menu-"
								+ a.btnCount++;
						c.symbol && (h = e.symbol(c.symbol, c.symbolX - j / 2,
