...
if ($.validator != null) {
	...
	$.extend($.validator.messages, {
		...
		spachar : "不能含有特殊字符！",
		equalTo : "请再次输入相同的值！",
		maxlength : $.validator.format("请输入一个长度最多是{0}的值！"),
		minlength : $.validator.format("请输入一个长度最少是{0}的值！"),
		rangelength : $.validator.format("请输入 一个长度介于{0}和{1}之间的值！"),
		range : $.validator.format("请输入一个介于{0}和{1}之间的值！"),
		max : $.validator.format("请输入一个最大为{0}的值！"),
		min : $.validator.format("请输入一个最小为{0}的值！"),
		digits : "只能输入正整数",
		<#--bigThan : "必须大于开始数值",
		prevRequire : "必须填写开始数值",
		nextRequire : "必须填写结束数值"-->
...
<#--
jQuery.validator.methods.bigThan = function(value, element, param) {
...
jQuery.validator.methods.prevRequire = function(value, element, param) {
	var prevNum = jQuery(param).val();
	if(prevNum != ""){
		if(value != ""){
			return true
		}
...
jQuery.validator.methods.nextRequire = function(value, element, param) {
	var nextNum = jQuery(param).val();
	if(nextNum != ""){
		if( value!= ""){
			return true
		}
